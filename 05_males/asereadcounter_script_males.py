...
final_output = "/data/Wilson_Lab/projects/placentas_VW_ASE/05_males/"
...
for placenta in config["dna_rna_males"]:
    for quadrant in quadrants:
        ...
        for chrom in chromosomes:
            ...
            # BAM file
            bam = bam_dirs[quadrant]+sample_id+"_HISAT_pair_trim_sort_mkdup_rdgrp_XY.bam"

            # Output file that combines vcf, bam, and chromosome
            outputfile = final_output+"HISAT/"+quadrant+"/chr"+chrom+"/"+sample_id+"_chr"+chrom+"_males.tsv"
                
            # Compose command and use it to compose a bash script
            command = gatk_path + " ASEReadCounter -R " + refXY + " --output " + outputfile + " --input " + bam + " --variant " + sites + " --min-depth-of-non-filtered-base 1 --min-mapping-quality 10 --min-base-quality 10"
