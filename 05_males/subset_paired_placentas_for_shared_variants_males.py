...
# When running the script, provide the chromosome as "X", not "chrX"
...
base = "/data/Wilson_Lab/projects/placentas_VW_ASE/05_males/allele_balance_tables/"

# Load config
with open("/data/Wilson_Lab/projects/placentas_VW_ASE/05_males/asereadcounter_config_males.json") as json_file:
...
for placenta in placentas:
    ...
    for quad in quadrants:
        sample_id = config["dna_rna_males"][placenta][quad]
        fname = f"{placenta}_{sample_id}_chr{chrom}_allele_balance.tsv"
    ...
    out_dir = "/data/Wilson_Lab/projects/placentas_VW_ASE/05_males/placentas_shared_variants/"
    os.makedirs(out_dir, exist_ok=True)

    out_file = os.path.join(
        out_dir,
        f"{placenta}_placentas_shared_variants_chr{chrom}.csv")
